#!/usr/bin/env bash
# Add the rate-snapshot column to time_entries on an existing TimeLog database.
set -euo pipefail
...
SQL="$INIT_DIR/13-entry-rate-snapshot.sql"
if [[ ! -f "$SQL" ]]; then
  echo "✕ Migration file not found: $SQL"
  exit 1
fi

echo "→ Applying entry rate-snapshot migration to database '$DB_NAME'..."
