...
**Обязательно для всех существующих установок** — миграция безопасности и
производительности (`17-hardening.sql`). Init-скрипты выполняются только при
первом создании volume, поэтому на уже работающей БД её нужно применить
вручную:

```bash
./scripts/migrate-db.sh          # применит все миграции 03+, включая 17
docker compose up -d --build     # обязательно: auth-сервис тоже изменился
```

Что она делает:

- снимает `DEFAULT PRIVILEGES` у роли `anon` — из-за них каждая таблица,
  добавленная после `10-rls-isolation.sql`, снова получала полный доступ для
  **неаутентифицированных** запросов PostgREST;
- убирает `FORCE ROW LEVEL SECURITY` с `app_users`: таблица работала только
  потому, что `POSTGRES_USER` в образе postgres — суперпользователь;
- добавляет составные индексы под реальные запросы синхронизации
  (`user_login` + сортировка);
- делает `user_login` `NOT NULL` (строки с `NULL` не видны ни RLS, ни
  синхронизации, но занимают первичный ключ);
- ставит `statement_timeout` и `idle_in_transaction_session_timeout` на роли.
